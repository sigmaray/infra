...
Ports are bound to `127.0.0.1` only — the proxy is not exposed to the public internet.
...
Host ports are bound to `127.0.0.1` — the proxy is not exposed to the public internet.
...
3. Run `./scripts/generate-3proxy-cfg.sh && docker compose up -d`.
...
- Do not publish ports on `0.0.0.0` without additional access controls (firewall, VPN, etc.).
